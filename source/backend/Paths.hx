...
class Paths
{
	...
	inline static public function sound(key:String, ?modsAllowed:Bool = true,?ignoreFolder:Bool=false):Sound {
	...
	inline static public function inst(song:String, ?modsAllowed:Bool = true,?isErect:Bool=false):Sound {
		if (isErect == true) {
			return returnSound('${formatToSongPath(song)}/erect/Inst', 'songs', modsAllowed);
		}
		return returnSound('${formatToSongPath(song)}/Inst', 'songs', modsAllowed);
	}

	inline static public function voices(song:String, postfix:String = null, ?modsAllowed:Bool = true,?isErect:Bool=false):Sound
	{
		var songKey:String;
		if (isErect == true) {
			songKey = '${formatToSongPath(song)}/erect/Voices';
		} else {
	...
	static public function image(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true,?ignoreFolder:Bool=false):FlxGraphic
	{
		key = Language.getFileTranslation('images/$key') + '.png';
		if (ignoreFolder == true) {
			key = Language.getFileTranslation(key) + '.png';
		}
