...
class FunkinLua {
	...
	public function new(scriptName:String) {
		...
		Lua_helper.add_callback(lua, "openSite", function(site:String) {
		...
		Lua_helper.add_callback(lua, "fromJson", function(?data:String="[]") { // https://wiki.multitheftauto.com/wiki/FromJSON
			return tjson.TJSON.parse(data);
		});

		Lua_helper.add_callback(lua, "toJson", function(?data:Dynamic,?ignoreTabs:Array<String>) { // https://wiki.multitheftauto.com/wiki/ToJSON
			return PsychJsonPrinter.print(data, ignoreTabs);
		});

		Lua_helper.add_callback(lua, "loadGraphic", function(variable:String, image:String, ?gridX:Int = 0, ?gridY:Int = 0,?ignoreFolder:Bool=false) {
			...
			{
				spr.loadGraphic(Paths.image(image,null,true,ignoreFolder), animated, gridX, gridY);
			}
		});
		...
		Lua_helper.add_callback(lua, "doTweenXY", function(tag:String, vars:String, XX:Dynamic,YY:Dynamic, duration:Float, ?ease:String = 'linear') {
			return oldTweenFunction(tag, vars, {x: XX, y: YY}, duration, ease, 'doTweenXY');
		});

		Lua_helper.add_callback(lua, "doTweenSX", function(tag:String, vars:String, value:Dynamic, duration:Float, ?ease:String = 'linear') {
			return oldTweenFunction(tag, vars+".scale", {x: value}, duration, ease, 'doTweenSX');
		});
		Lua_helper.add_callback(lua, "doTweenSY", function(tag:String, vars:String, value:Dynamic, duration:Float, ?ease:String = 'linear') {
			return oldTweenFunction(tag, vars+".scale", {y: value}, duration, ease, 'doTweenSY');
		});
		Lua_helper.add_callback(lua, "doTweenSXY", function(tag:String, vars:String, XX:Dynamic,YY:Dynamic, duration:Float, ?ease:String = 'linear') {
			return oldTweenFunction(tag, vars+".scale", {x: XX, y: YY}, duration, ease, 'doTweenSXY');
		});
		...
		Lua_helper.add_callback(lua, "toColor", function(?r:Int=255,?g:Int=255,?b:Int=255,?a:Int=255) {
		...
		Lua_helper.add_callback(lua, "updateTimer", function(tag:String,elapsed:Float) {
		...
		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, ?image:String = null, ?x:Float = 0, ?y:Float = 0,?ignoreFolder:Bool=false) {
			tag = tag.replace('.', '');
			LuaUtils.destroyObject(tag);
			var leSprite:ModchartSprite = new ModchartSprite(x, y);
			if(image != null && image.length > 0)
			{
				leSprite.loadGraphic(Paths.image(image,null,true,ignoreFolder));
			}
		...
		Lua_helper.add_callback(lua, "playMusic", function(sound:String, ?volume:Float = 1, ?loop:Bool = false,?ignoreFolder:Bool=false) {
			FlxG.sound.playMusic(Paths.music(sound,true,ignoreFolder), volume, loop);
		});

		Lua_helper.add_callback(lua, "playSound", function(sound:String, ?volume:Float = 1, ?tag:String = null, ?loop:Bool = false,?ignoreFolder:Bool=false) {
			if(tag != null && tag.length > 0)
			{
				...
				variables.set(tag, FlxG.sound.play(Paths.sound(sound,true,ignoreFolder), volume, loop, null, true, function()
				{
		...
		Lua_helper.add_callback(lua, "setSongTime", function(time:Float=0) {
			return game.setSongTime(time);
		});

		Lua_helper.add_callback(lua, "setSongPitch", function(Pitch:Float=0) {
			FlxG.sound.music.pitch = Pitch;
			game.opponentVocals.pitch = Pitch;
			game.vocals.pitch = Pitch;
			return Pitch;
		});

		Lua_helper.add_callback(lua, "getSongTime", function(Pitch:Float=0) {
			return FlxG.sound.music.time;
		});

		Lua_helper.add_callback(lua, "pauseSong", function(isPause:Bool=true) {
			if (isPause == true) {
				FlxG.sound.music.pause();
				game.vocals.pause();
