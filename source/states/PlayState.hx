...
class PlayState extends MusicBeatState
{
	...
	public var isCoolSus:Bool = true;
	...
	override public function create()
	{
		...
		// for (folder in Paths.getSharedPath('scripts/'))
		// 	for (file in FileSystem.readDirectory(folder))
		// 	{
		// 		#if LUA_ALLOWED
		// 		if(file.toLowerCase().endsWith('.lua'))
		...
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'data/songs/$songName/scripts/'))
			for (file in FileSystem.readDirectory(folder))
			{
				#if LUA_ALLOWED
				if(file.toLowerCase().endsWith('.lua'))
		...
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'data/states/luaStates/$songName/scripts/'))
			for (file in FileSystem.readDirectory(folder))
			{
				#if LUA_ALLOWED
				if(file.toLowerCase().endsWith('.lua'))
		...
		for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), 'data/states/luaStates/$songName/'))
			for (file in FileSystem.readDirectory(folder))
			{
				#if LUA_ALLOWED
				if(file.toLowerCase().endsWith('.lua'))
	...
	public dynamic function updateScoreText()
	{
		...
		var formatScore:String = FlxStringUtil.formatMoney(songScore, false, true);
		if(!instakillOnMiss) tempScore = Language.getPhrase('score_text', 'Score: {1} •  Combo Breaks: {2} • Accuracy: {3}', [formatScore, songMisses, str]);
		else tempScore = Language.getPhrase('score_text_instakill', 'Score: {1} | Rating: {2}', [formatScore, str]);
	...
	private function generateSong():Void
	{
		...
		try
		{
			if (songData.needsVoices)
			{
				...
				if (storyDifficultyText == "erect" || storyDifficultyText == "nightmare") {
					var playerVocals = Paths.voices(songData.song, (boyfriend.vocalsFile == null || boyfriend.vocalsFile.length < 1) ? 'Player' : boyfriend.vocalsFile,true,true);
		...
		{
			if (storyDifficultyText == "erect" || storyDifficultyText == "nightmare") {
			inst.loadEmbedded(Paths.inst(songData.song,true,true));
			} else {
		...
		{
			if (storyDifficultyText == "erect" || storyDifficultyText == "nightmare") {
			var eventsChart:SwagSong = Song.getChart('events-erect', songName);
			if(eventsChart != null)
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			if (!isStoryMode && !skipArrowStartTween)
			{
				babyArrow.y -= 180;
				babyArrow.alpha = 0;
				trace(ClientPrefs.data.strumAlpha);
				FlxTween.tween(babyArrow, {y: babyArrow.y + 180, alpha: targetAlpha}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i)});
